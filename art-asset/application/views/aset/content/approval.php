...
<header><h3>Need Approval</h3>
</header>

<div id="tabel_data">
<?php $this->load->view('/aset/content/approvalTable'); ?>
</div>
...
<div id="dialog-decline" title="Confirmation"> 
	<p>
		<input type="hidden" value='' id="del_id" name="del_id">
		<img src='<?php echo base_url()?>images/icn_alert_error.png'> Are You Sure to <strong>Decline</strong>?</p> 
</div> 
...
<script type="text/javascript" charset="utf-8">
		...
		$( "#dialog-decline" ).dialog({
			...
			buttons: {
				"Decline": function() {
					...
					$.ajax({
						type: "POST",
						url : "<?php echo site_url('aset/adminFunc/dec_pr')?>",
						data: del_id,
						success: function(msg){
							$('#tabel_data').html(msg);
							$.jGrowl("<h4>Purchase Request Declined</h4>");	
							$('#dialog-decline' ).dialog( "close" );
						}
				  	});
	...
	$(".del-decline").live("click",function(){
    	var kode = $(this).attr("kode");
    	var info = 'kode=' + kode;
    	$('#del_id').val(kode);
		
    	$( "#dialog-decline" ).dialog( "open" );
